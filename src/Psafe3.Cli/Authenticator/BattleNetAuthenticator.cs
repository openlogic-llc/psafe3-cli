...
namespace WinAuth
{
    ...
    public class BattleNetAuthenticator : Authenticator
    {
        ...
        public void Enroll()
        {
            ...
            // TODO: (RAK) Consider adding this back in
            ...
            rsa.Init(true,
                new RsaKeyParameters(false, new BigInteger(ENROLL_MODULUS, 16), new BigInteger(ENROLL_EXPONENT, 16)));
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        ...
                        using (var bs = response.GetResponseStream())
                        {
                            ...
                            {
                                throw new InvalidEnrollResponseException(string.Format(
                                    "Invalid response data size (expected 45 got {0})", responseData.Length));
                            }
                        }
                    }
                }
        ...
        /// <summary>
        ///     Synchronise this authenticator's time with server time. We update our data record with the difference from our UTC
        ///     time.
        /// </summary>
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        ...
                        using (var bs = response.GetResponseStream())
                        {
                            ...
                            {
                                throw new InvalidSyncResponseException(string.Format(
                                    "Invalid response data size (expected " + SYNC_RESPONSE_SIZE + " got {0}",
                                    responseData.Length));
                            }
                        }
                    }
            ...
            string restoreCode)
        {
                        ...
                        using (var bs = response.GetResponseStream())
                        {
                            ...
                            if (challenge.Length != RESTOREINIT_BUFFER_SIZE)
                            {
                                throw new InvalidRestoreResponseException(
                                    string.Format("Invalid response data size (expected 32 got {0})",
                                        challenge.Length));
                            }
                        }
                    }
            ...
            catch (WebException we)
            {
                ...
                {
                    throw new InvalidRestoreResponseException(
                        string.Format("No response from server ({0}). Perhaps maintainence?", code));
                }

                throw new InvalidRestoreResponseException(string.Format("Error communicating with server: {0} - {1}",
                    code, ((HttpWebResponse)we.Response).StatusDescription));
            }
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        using (var bs = response.GetResponseStream())
                        {
                            ...
                            {
                                throw new InvalidRestoreResponseException(string.Format(
                                    "Invalid response data size (expected " + RESTOREVALIDATE_BUFFER_SIZE + " got {0})",
                                    secretKey.Length));
                            }
                        }
                    }
            ...
            if (serial.Length == 14)
            {
                Serial = serial.Substring(0, 2).ToUpper() + "-" + serial.Substring(2, 4) + "-" +
                         serial.Substring(6, 4) + "-" + serial.Substring(10, 4);
            }
            else
            {
        ...
        public override bool ReadExtraXml(
            XmlReader reader,
            string name)
        {
            switch (name)
        ...
        protected override void WriteExtraXml(
            XmlWriter writer)
        {
        ...
        private static string GetMobileUrl(
            string region)
        {
        ...
        /// <summary>
        ///     Calculate the restore code for an authenticator. This is taken from the last 10 bytes of a digest of the serial and
        ///     secret key,
        ...
        public override string SecretData
        {
            ...
            get => base.SecretData + "|" + ByteArrayToString(Encoding.UTF8.GetBytes(Serial));
        ...
        /// <summary>
        ///     Convert a char to a byte but with appropriate mapping to exclude I,L,O and S. E.g. A=10 but J=18 not 19 (as I is
        ///     missing)
        /// </summary>
        /// <param name="c">char to convert.</param>
        /// <returns>byte value of restore code char</returns>
        private static byte ConvertRestoreCodeCharToByte(
            char c)
        {
            if (c >= '0' && c <= '9')
            {
        ...
        private static char ConvertRestoreCodeByteToChar(
            byte b)
        {
            var index = b & 0x1f;
            if (index <= 9)
